...
func GetOssSign(c *gin.Context)  {
	...
	response := get_policy_token(config.AccessKeyId,config.AccessKeySecret)
...
var host string = "http://xkws.oss-cn-hangzhou.aliyuncs.com"
...
func get_policy_token(accessKeyId,accessKeySecret string) PolicyToken {
	now := time.Now().Unix()
